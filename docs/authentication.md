...
Passphrases are defined in `vaimopages.config` under the `branches` list — one passphrase per branch. The app reads this list at authentication time and matches the submitted passphrase.

```
User submits passphrase "abc123"
  └──▶ app reads vaimopages.config
...
Configured in `vaimopages.config`:
...
> The `AUTH_PASSPHRASE` environment variable used in earlier versions is no longer required. Passphrases are now defined entirely in `vaimopages.config`.

## Security Notes

- Passphrases are compared server-side only, inside the NextAuth credentials handler. They are never sent to the client.
- Passphrases are stored as plain text in `vaimopages.config`. Treat the config file as a secret and restrict access to the docs repository accordingly.
